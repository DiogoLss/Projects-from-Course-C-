...
namespace CursoCSharp.TopicosAvancados
{
    internal class LINQ2
    {
        public static void Executar()
        {
            ...
            else { Console.WriteLine($"{fulano.Nome} {fulano.Nota}"); }
            ...
            else { Console.WriteLine($"{sicrano.Nome} {sicrano.Nota}"); }

            var outraAna = alunos.LastOrDefault(aluno => aluno.Nome.Equals("Ana"));
            if(outraAna == null) { Console.WriteLine(" Aluno Inexistente"); }
            else Console.WriteLine(outraAna.Nota);

            var exemploSkip = alunos.Skip(1).Take(3);
            foreach (var item in exemploSkip) { Console.WriteLine(item.Nome); }
            ...
            var mediaDaTurmaAprovados = alunos.Where(a => a.Nota >= 7.0).Average(aluno => aluno.Nota);
